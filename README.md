...
| :--- | :--- | :--- | :--- |
| **纯内存读取 (GET)** | 1,612,903 RPS (0.87ms) | **1,718,213 RPS** (0.83ms) | **超越原生 Redis ~10.5 万 RPS！** 多线程结合 1024 个 Shard 分段读锁 (`RwLock::read`)，彻底释放多核并发能力。 |
| **纯内存写入 (SET)** | 846,023 RPS (1.79ms) | **1,096,491 RPS** (1.39ms) | **领先原生 Redis ~25 万 RPS！** 64字节缓存行对齐 (`#[repr(align(64))]`) + O(1) 全局原子记账，彻底根除伪共享与死锁。 |
