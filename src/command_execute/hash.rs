...
impl CommandExecutor for HSetCommand {
    async fn execute(
        ...
        if let Some(LockedDb::Write(map)) = db_lock {
            ...
            Err(KvError::ProtocolError("HSET requires a write lock".into()))
        }
    }
}
...
impl CommandExecutor for HGetCommand {
    async fn execute(
        ...
        if let Some(LockedDb::Read(map)) = db_lock {
            ...
            Err(KvError::ProtocolError("HGET requires a read lock".into()))
        }
    }
}
...
impl CommandExecutor for HDelCommand {
    async fn execute(
        ...
        if let Some(LockedDb::Write(map)) = db_lock {
            ...
            Err(KvError::ProtocolError("HDEL requires a write lock".into()))
        }
    }
}
