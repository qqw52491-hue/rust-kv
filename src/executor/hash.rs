use crate::{
    ...
    error::{Frame, HDelCommand, HGetCommand, HSetCommand, KvError},
...
impl Executor for HSetCommand {
    async fn execute(&self, ctx: CommandContext) -> Result<Frame, KvError> {
        let added_count = {
            ...
            map.insert(
                self.key.clone(),
                ValueEntry::new(Value::Hash(hash_map, elements_heap), expires_at),
            );

            // 就在锁将要释放前发送 AOF
            ctx.send_aof(&crate::error::Command::HSet(self.clone()))
                .await;
    ...
    async fn execute(&self, ctx: CommandContext) -> Result<Frame, KvError> {
        ...
        match map.select(&self.key) {
            Some(entry) => match &entry.data {
                Value::Hash(hash_map, _) => match hash_map.get(&self.field) {
...
impl Executor for HDelCommand {
    async fn execute(&self, ctx: CommandContext) -> Result<Frame, KvError> {
        let removed_count = {
            ...
            ctx.send_aof(&crate::error::Command::HDel(self.clone()))
                .await;
            removed_count
        };
