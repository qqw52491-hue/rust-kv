...
mod tests {
    ...
    use crate::context::{CONN_STATE, ConnectionState, ConnectionContent};
    use crate::error::{Command, Frame, LPushCommand, BLPopCommand, LPopCommand, KvError};
    ...
    fn create_dummy_ctx(db: Db) -> CommandContext {
        ...
        let lua_sender = LuaRouter { senders: vec![lua_sender_tx] };
    ...
    async fn test_blpop_timeout() {
        ...
        let res = CONN_STATE.scope(state, async {
    ...
    async fn test_blpop_wakeup() {
        ...
        let handle = tokio::spawn(async move {
            CONN_STATE.scope(state, async move {
                blpop.execute(ctx1).await.unwrap()
            }).await
        });
        ...
        CONN_STATE.scope(state2, async move {
